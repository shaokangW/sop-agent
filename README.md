...
一个 **SOP（声明式流程）+ harness（运行时引擎）** 的 agent 框架：把"做什么"（流程）和"怎么执行"（agent 自主）解耦，支持 SOP 编排、自主 agent、多轮对话三种模式，带工具调用、人在回路审批、流式输出（含思考过程）和 Web/TUI 双前端。
...
harness   engine / autonomous / interactive / approval / events / transitions / tui
LLM 抽象  base(Protocol) / openai_provider / registry / router  (多 provider + 流式 + reasoning)
工具层    base / registry / executor / builtin(8 工具) / mcp_client
SOP 模型  schema(Pydantic) / loader(YAML 解析 + 变量插值)
配置      config.py (env + opencode.json 读取百炼凭证)
...
- **工具集**（参照 opencode/claude-code）：`read_file` `write_file` `edit_file` `list_dir` `grep` `bash` `web_fetch` `todo_write` + MCP 协议接入；危险工具（write/edit/bash）自动需审批
- **人在回路审批**：动作级（tool call）+ 阶段级（step/subgoal）；三选项「本次允许 / 对话允许 / 拒绝」
- **流式 + 思考过程**：原生 tool_calls 循环 + `reasoning_content` 流式（GLM-5.2 思考链）
...
- **Web UI**：消息分块（user/assistant 配色）+ markdown 渲染 + 思考块 + 工具调用两级折叠 + 侧边栏（模型/使用情况/工具汇总）+ session 管理（新建/历史/继续）+ 三模式切换
...
百炼 GLM-5.2 凭证：若本地装了 opencode 并 `/connect` 过百炼，sop-agent 自动从 `~/.config/opencode/opencode.json` 读取 `bailian` provider 的 apiKey + baseURL。也可用环境变量覆盖：
...
顶部切模式：💬 对话（多轮）/ 🎯 任务（自主）/ 📋 SOP（粘 YAML 运行）。
...
├── server.py               FastAPI + Web UI (/chat /task /run /approve)
├── config.py               Settings（百炼凭证从 opencode 读）
├── sop/                    SOP 模型 + loader
├── llm/                    LLM 抽象（provider/router/stream/reasoning）
├── tools/                  工具层（builtin 8 工具 + MCP + executor）
...
sops/                       SOP 示例
tests/                      测试（60+，覆盖 loader/engine/transitions/approval/autonomous/tools/server/tui）
```
